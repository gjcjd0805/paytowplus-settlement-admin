...
export default function SettlementAmountDistributorPage() {
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      console.error("총판 정산금액 조회 오류:", error)
  ...
  // 재귀적으로 계층 구조 렌더링 (총판 > 대리점)
  const renderHierarchy = (items: HierarchicalStatistics[], baseLevel: number = 2): JSX.Element[] => {
        ...
        <table className="w-full border-collapse bg-white">
          <thead className="bg-[#6B7D8F] text-white text-xs font-semibold">
            <tr>
              ...
              <th colSpan={4} className="border border-white px-1 py-1.5">
                구분
              </th>
            ...
            {statistics.length === 0 ? (
              ...
              <>
                ...
                <tr className="bg-yellow-50 font-bold border-t-2 border-gray-400">
                  <td className="border-r border-gray-300 px-2 py-2 text-center" colSpan={2}>
                    전체 합계
                  </td>
                  <td className="border-r border-gray-300 px-1 py-2 text-right">
