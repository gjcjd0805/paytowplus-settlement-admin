...
import type { MerchantSettlement, MerchantSettlementTotalSummary } from "@/lib/api/types"
...
export default function MerchantSettlementPeriodPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await settlementsApi.findMerchantsByPeriod(params)
    ...
    <table className="w-full border-collapse bg-white">
      <thead className="bg-[#6B7D8F] text-white text-xs font-semibold">
        <tr>
          ...
          <th colSpan={4} className="border border-white px-1 py-1.5">차감매출(정산후취소)</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[70px]">정산차감</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[80px]">정산금액</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[70px]">이체수수료</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[70px]">입금금액</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[70px]">은행명</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[90px]">계좌번호</th>
          <th rowSpan={2} className="border border-white px-1 py-2 min-w-[70px]">예금주</th>
        </tr>
        <tr>
          <th className="border border-white px-1 py-1.5 min-w-[50px]">건수</th>
          <th className="border border-white px-1 py-1.5 min-w-[80px]">거래금액</th>
          <th className="border border-white px-1 py-1.5 min-w-[70px]">수수료</th>
          <th className="border border-white px-1 py-1.5 min-w-[80px]">정산금액</th>
          <th className="border border-white px-1 py-1.5 min-w-[50px]">건수</th>
          <th className="border border-white px-1 py-1.5 min-w-[70px]">취소금액</th>
          <th className="border border-white px-1 py-1.5 min-w-[70px]">수수료</th>
          <th className="border border-white px-1 py-1.5 min-w-[80px]">차감완료금액</th>
        </tr>
      </thead>
      <tbody className="text-xs text-gray-700">
        {settlements.length === 0 ? (
          <tr>
            <td colSpan={17} className="text-center py-10 text-gray-500">검색 결과가 없습니다.</td>
          </tr>
        ) : (
          settlements.map((row, index) => (
            <tr key={row.merchantId} className="border-b border-gray-300 hover:bg-gray-50">
              ...
              <td className="border-r border-gray-300 px-1 py-2 text-right">{formatAmount(row.depositAmount)}</td>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xl text-gray-600">
        ...
        <span className="font-semibold text-gray-800">가맹점 정산금액 조회(기간별)</span>
      </div>
