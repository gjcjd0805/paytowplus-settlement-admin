...
import type { MerchantSales, MerchantSalesTotalSummary } from "@/lib/api/types"
...
export default function MerchantSalesPage() {
  ...
  // 필수 상태
  const [sales, setSales] = useState<MerchantSales[]>([])
  const [isLoading, setIsLoading] = useState(false)
  const [currentPage, setCurrentPage] = useState(0) // 0-based
  const [totalPages, setTotalPages] = useState(0)
  const [totalElements, setTotalElements] = useState(0)
  const [pageSize, setPageSize] = useState(15)
  const [totalSummary, setTotalSummary] = useState<MerchantSalesTotalSummary | null>(null)

  // 검색 조건 상태 (초기값: 오늘 날짜)
  const { start: todayStart, end: todayEnd } = getDateRange("오늘")
  const [dateType, setDateType] = useState<"settlement" | "payment">("settlement") // 정산일자 or 거래일자
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await salesApi.findMerchants(params)
    ...
    <table className="w-full border-collapse bg-white">
      <thead className="bg-[#6B7D8F] text-white text-xs font-semibold">
        <tr>
          ...
          <th className="border border-white px-1 py-2 min-w-[50px]">결제건수</th>
          <th className="border border-white px-1 py-2 min-w-[90px]">승인금액</th>
          <th className="border border-white px-1 py-2 min-w-[80px]">취소금액</th>
          <th className="border border-white px-1 py-2 min-w-[90px]">거래금액</th>
          <th className="border border-white px-1 py-2 min-w-[80px]">정산금액</th>
          <th className="border border-white px-2 py-2 min-w-[200px]">AGENT</th>
        </tr>
      </thead>
      <tbody className="text-xs text-gray-700">
        {sales.length === 0 ? (
          <tr>
            <td colSpan={8} className="text-center py-10 text-gray-500">검색 결과가 없습니다.</td>
          </tr>
        ) : (
          sales.map((row, index) => (
            <tr key={row.merchantId} className="border-b border-gray-300 hover:bg-gray-50">
              ...
              <td className="border-r border-gray-300 px-1 py-2 text-center text-blue-600 font-medium">{row.count}</td>
              ...
              <td className="px-2 py-2 text-left text-xs">{row.companyNamePath || "-"}</td>
            </tr>
          ))
        )}
      </tbody>
      {/* 합계 행 */}
      {totalSummary && sales.length > 0 && (
        <tfoot className="bg-[#6B7D8F] text-white text-xs font-bold">
          <tr>
            <td colSpan={2} className="border border-white px-1 py-2 text-center">합계</td>
            <td className="border border-white px-1 py-2 text-center text-yellow-300">{totalSummary.totalCount}</td>
            ...
            <td className="border border-white px-1 py-2 text-center"></td>
          </tr>
        </tfoot>
      )}
    </table>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xl text-gray-600">
        ...
        <span className="font-semibold text-gray-800">가맹점 매출금액 조회</span>
      </div>
